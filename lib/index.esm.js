...
function commonElementHandler(parent, newChild, passiveChild, rawMethod) {
    const appName = newChild?.name || newChild.__MICRO_APP_NAME__
    if (newChild?.__MICRO_APP_NAME__ || appName) {
...
function runScript(url, app, info, isDynamic, callback) {
    ...
    try {
        ...
        if (app.remoteScript) {
            let scriptElement = pureCreateElement('script');
            scriptElement.name = app.name;
            scriptElement.setAttribute('name', app.name);
            runCode2RemoteScript(url, code, info.module, scriptElement, callback);
            if (isDynamic) {
        ...
        } else if (app.inline || info.module) {
...
function runDynamicRemoteScript(url, info, app, originScript) {
    ...
    let replaceElement;
    if (app.remoteScript || (app.inline || info.module)) {
    ...
    if (app.remoteScript) {
        replaceElement.name = app.name;
        runCode2RemoteScript(url,'', info.module, replaceElement, dispatchScriptOnLoadEvent);
    } else {
...
function runCode2RemoteScript(url, code, module, scriptElement, callback) {
    if (module) {
        // module script is async, transform it to a blob for subsequent operations
        const blob = new Blob([code], { type: 'text/javascript' });
        scriptElement.src = URL.createObjectURL(blob);
...
function bindScope(url, app, code, info) {
    ...
    if (app.sandBox && !info.module) {
        ...
        if (app.remoteScript || code.startsWith(';(function(proxyWindow)')) {
            return code
        }
...
class CreateApp {
    constructor({ name, url, ssrUrl, container, inline, scopecss, useSandbox, baseroute, remoteScript}) {
        ...
        this.remoteScript = remoteScript !== null && remoteScript !== void 0 ? remoteScript : null;
    ...
    class MicroAppElement extends HTMLElement {
        ...
        handleCreateApp() {
            ...
            const instance = new CreateApp({
                ...
                url: this.appUrl,
                ssrUrl: this.ssrUrl,
                remoteScript: this.getDisposeResult('remoteScript'),
